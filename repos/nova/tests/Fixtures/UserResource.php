...
class UserResource extends Resource
{
    ...
    public function fields(Request $request)
    {
        return [
            ...
            $this->when(
                $_SESSION['nova.user.cover'] ?? false,
                function () {
    ...
    public function rolesFields()
    {
        ...
        return BelongsToMany::make('Roles', 'roles', RoleResource::class)->referToPivotAs(
            $_SERVER['nova.user.rolePivotName'] ?? null
        )->fields(
            function () {
                return [
                    tap(
    ...
    public function actions(Request $request)
    {
        return [
            ...
            (new UnauthorizedAction)->canSee(
                function ($request) {
                    return false;
                }
            ),
            (new UnrunnableAction)->canSee(
                function ($request) {
                    return true;
                }
            )->canRun(
            ...
            (new UnrunnableDestructiveAction)->canRun(
                function ($request, $model) {
                    return false;
                }
            ),
    ...
    public function filters(Request $request)
    {
        return [
            ...
            (new CustomKeyFilter)->canSee(
                function ($request) {
                    return $_SERVER['nova.customKeyFilter.canSee'] ?? true;
                }
            ),

            (new ColumnFilter('id'))->canSee(
                function ($request) {
                    return $_SERVER['nova.columnFilter.canSee'] ?? true;
                }
            ),

            (new CreateDateFilter)->firstDayOfWeek(4)->canSee(
                function ($request) {
