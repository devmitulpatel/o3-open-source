    ...
    public function resource_can_be_updated_to_new_parent()
    {
        $this->whileSearchable(
            function () {
                ...
                $this->browse(
                    function (Browser $browser) use ($comment) {
    ...
    public function morph_to_field_should_honor_custom_polymorphic_type()
    {
        $this->whileSearchable(
            function () {
                ...
                $this->browse(
                    function (Browser $browser) use ($comment, $link) {
                        $browser->loginAs(User::find(1))
                            ->visit(new Update('comments', $comment->id))
                            ...
                            ->within(
                                '@commentable-search-input',
                                function ($browser) use ($link) {
    ...
    public function morph_to_field_should_ignore_query_parameters_when_editing()
    {
        $this->whileSearchable(
            function () {
                ...
                $this->browse(
                    function (Browser $browser) use ($comment, $post) {
                        $browser->loginAs(User::find(1))
                            ->visit(
                                new Update(
                                    'comments', $comment->id, [
