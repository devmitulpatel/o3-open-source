...
abstract class Trend extends RangedMetric
{
    ...
    protected function aggregate($request, $model, $unit, $function, $column, $dateColumn = null)
    {
        ...
        $results = array_merge(
            $possibleDateResults,
            $results->mapWithKeys(
                function ($result) use ($request, $unit) {
                    return [
     ...
     *
     * @param Chronos $startingDate
     * @param Chronos $endingDate
     * @param string $unit
    ...
    protected function formatAggregateMonthDate($result)
    {
        ...
        return with(
            Chronos::create((int)$year, (int)$month, 1),
            function ($date) {
                return __($date->format('F')) . ' ' . $date->format('Y');
            }
        );
